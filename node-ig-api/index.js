...
//var tokensDir = path.join(__dirname, '../../tmp/tokens.json'); RC my line
//var tokens = require(tokensDir);

//RC Changing to environment variables instead of reading/writing from file.
			...
			_request('GET', '/session/encryptionKey') // retrieve encryptionKey and timeStamp for encryption
				...
				.then(r => {
					if (r.status !== 200) {
						...
						process.env.IG_TOKENS_EXP = new Date().getTime() + 43200000;
						//tokens['x-request-id'] = r.headers['x-request-id'];
						process.env.X_REQUEST_ID = r.headers['x-request-id'];
						//tokens['x-security-token'] = r.headers['x-security-token'];
						process.env.IG_XST = r.headers['x-security-token'];
						//tokens.cst = r.headers.cst;
						process.env.IG_CST = r.headers.cst;
						//tokens.lightstreamerEndpoint = r.body.lightstreamerEndpoint;
						process.env.IG_LIGHTSTREAMER_END_POINT = r.body.lightstreamerEndpoint;
						//tokens.currentAccountId = r.body.currentAccountId;
						process.env.CURRENT_ACCOUNT_ID = r.body.currentAccountId;
						res(r.body);
						/*
		...
		get('/positions')
			.then(r => {
				if (r.status !== 200) {
					...
					res(JSON.parse(JSON.stringify(r.body)));
				}
			});
	});
}
...
function deal(ticket) {
	return new Promise((res, rej) => {
		// Constraints:
		if (['AUD','EUR', 'GBP', 'USD'].indexOf(ticket.currencyCode) === -1) throw new Error('currencyCode has to be EUR, GBP or USD');
...
// Historical prices
function histPrc(epic, resolution) {

	/**
	* Return the latest resolution period of data
	 * @param {string} epic
	 ...
	 */
	return new Promise((res, rej) => {
		console.log('API DATA REQUEST = /prices/' + epic + '?resolution=' + resolution + '&max=1');
		get('/prices/' + epic + '?resolution=' + resolution + '&max=1', 3)
			.then(r => {
				if (r.status !== 200) {
					//console.log('API DATA REQUEST ERORR =)',r);
					rej(r);
				} else {
					//console.log('API DATA REQUEST SUCESS =)',r.body);
					res(r.body);
				}
			})
			.catch(e => {
